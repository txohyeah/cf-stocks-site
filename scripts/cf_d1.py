#!/usr/bin/env python3
"""创建 D1 数据库并绑定到 Pages 项目 stocks-site，再执行 schema/seed SQL。
token 从 .secrets/cf_api_token 读取，不回显。
用法：
...
def split_statements(sql):
    """按 ';\\n' 切分语句（与 exec/query 共用；数据中无 ASCII 分号结尾换行）。"""
    return [s.strip() for s in re.split(r';\s*\n', sql) if s.strip()]


def exec_sql_file(db, path):
    sql = open(path).read()
    # 按 ";\n" 切分语句（数据中无 ASCII 分号结尾换行）
